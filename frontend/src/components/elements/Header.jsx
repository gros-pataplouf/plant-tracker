...
export default function Header({ props }) {
  ...
  return (
    <div
      className="absolute top-0 h-[10vh] bg-emerald-900 space-4 w-screen"
      id="header"
    >
      {!isLoggedIn ? (
        <div className="py-2 bg-emerald-950 h-[2.5rem] max-h-[8vh] min-h-[4vh]">
          <button className="px-2 ml-6 font-bold rounded-lg text-emerald-950 bg-yellow-50">
            <Link to={"login/"}>Log in</Link>
          </button>
          <span className="text-yellow-50"> or</span>
          <button className="px-2 font-bold rounded-lg text-yellow-50">
      ...
      <div id="titleContainer" className="flex pl-4">
        <svg
          className="m-2"
        ...
        <h2 className="p-4 text-5xl text-yellow-50">Planttracker</h2>
      </div>
      <img
        className="fixed z-30 p-2 rounded-full bg-emerald-950 top-2 right-2"
        src={hamburger}
        alt=""
        onClick={toggleNav}
      />
