...
export default function Activate() {
  ...
  axiosInstance.get(`accounts/activate/${activationToken}`).then((res) => {
    if (res.status === 204) {
  ...
  .catch((err) => {
    setMessage("Oh no! An error has occured. Please request another activation token.");
